/*
 * =================================================================================================
 * Volto • Product Videos by ARTICLE (Catalog + Modal) • v1.0
 *
 * Что делает:
 * 1) Каталог (карточки товаров):
 *    - Берёт ключ товара по артикулу:
 *        .product .product__article (hidden) → например "a1012"
 *    - Проверяет наличие видео только по пути:
 *        /upload/video/{article}.mp4
 ...
 *    - Не плодит дубли при SPA/перерендерах (data-метки на карточке + MutationObserver)
 *    - Кеширует HEAD-результат по артикулу (не спамит запросами)
 *
 * 2) Модалка товара (SweetAlert2 + Swiper):
 *    - При открытии модалки читает артикул:
 *        .product-modal-fields .art_val → например "a1012"
 *    - Добавляет ВИДЕО отдельным слайдом №1 (картинки не трогает)
 *    - Автовоспроизведение (muted) + пауза, когда слайд не активен, и play когда активен
 *    - Не спамит HEAD-проверками (seen-метка + кеш по артикулу)
 ...
 * - .video-indicator скрыт (display:none)
 * - .mdx-modal-video-el имеет border-radius:12px
 * =================================================================================================
 */
...
$(document).ready(function () {
  const CFG = {
    ...
    metaTimeoutMs: 4000, // таймаут ожидания loadedmetadata
    ...
    dataHas: "mdxVideoHas",
    dataKey: "mdxVideoKey", // какой article обработали (для самоконтроля)

    // артикул
    selCatalogArticle: ".product__article",
    selModalArticle: ".product-modal-fields .art_val",

    // модалка
    selModal: ".swal2-product-modal-container .product-modal[data-id]",
    sliderIdTpl: (id) => `#product_${id}_slider`, // ID нужен только чтобы найти слайдер (как раньше)
  ...
  LOG.info("ProductVideos by ARTICLE v1.0: старт.");

  // ===== CSS инъекция (без правок HTML) =====
  (function injectCSSOnce() {
    const id = "mdx_product_videos_article_v10_css";
  ...
  // Кеш HEAD по article: чтобы одинаковые товары не спамили запросами
  const HEAD_CACHE = new Map(); // article -> { exists:boolean, head:{...}, ts:number }
  ...
  function readArticleFromCatalogProduct($product) {
    const raw = $product.find(CFG.selCatalogArticle).first().text();
    return (raw || "").trim();
  }

  function readArticleFromModal(modalEl) {
    const el = modalEl.querySelector(CFG.selModalArticle);
    return (el && el.textContent ? el.textContent : "").trim();
  }

  function buildVideoUrlByArticle(article) {
    return `${CFG.videoRoot}${article}${CFG.videoExt}`;
  }

  async function checkVideoExistsByArticle(article) {
    if (!article) return { exists: false, head: null, url: "" };

    if (HEAD_CACHE.has(article)) {
      return HEAD_CACHE.get(article);
    }

    const url = buildVideoUrlByArticle(article);
    const head = await abortableHead(url, CFG.headTimeoutMs);
    ...
    const res = { exists, head, url, ts: Date.now() };
    HEAD_CACHE.set(article, res);
    return res;
  }
  ...
  function safeIdFromArticle(article) {
    // чтобы id не ломался: всё кроме букв/цифр/дефиса/подчёркивания → _
    return String(article)
      .trim()
      .replace(/[^a-zA-Z0-9_-]/g, "_");
  }

  function replaceImageWithVideoByArticle(
    $product,
    $imgContainer,
    $img,
    videoUrl,
    ...
    const videoId = `mdx_product_video_${safeIdFromArticle(article)}`;
    ...
    function rollback(reason) {
      ...
      if (CFG.debug) LOG.warn(`rollback: article ${article} (${reason})`);
    }
    ...
    videoEl.addEventListener(
      ...
      function () {
        ...
        if (CFG.debug)
          LOG.success(`Видео подтверждено и включено: article ${article}`);
      },
      { once: true },
    );
  ...
  async function initProductVideosCatalog() {
    ...
    try {
      ...
      for (let i = 0; i < $products.length; i++) {
        ...
        // Берём article как ключ
        const article = readArticleFromCatalogProduct($product);
        if (!article) continue;

        // Дубль-защита
        if (
          $product.data(CFG.dataDone) === true &&
          $product.data(CFG.dataKey) === article
        )
          continue;
        $product.data(CFG.dataDone, true);
        $product.data(CFG.dataKey, article);
        ...
        const chk = await checkVideoExistsByArticle(article);
        if (!chk.exists) {
          cleanupOverlays($imgContainer);
          continue;
        }
        ...
        existsYes++;
        replaceImageWithVideoByArticle(
          $product,
          $imgContainer,
          $img,
          chk.url,
      ...
      // Итоговый лог (вместо спама)
      if (CFG.debug)
        LOG.info(
          `catalog: scanned=${scanned}, video_exists=${existsYes}, injected_attempts=${injected}`,
        );
    } finally {
      INIT_LOCK = false;
    }
  ...
  function ensureModalVideoSlideByArticle(article, sliderEl) {
    ...
    // self-heal: если Swiper пересобрал DOM и наш слайд пропал — разрешаем вставку заново
    if (done && !hasOurSlide) {
    ...
    video.src = buildVideoUrlByArticle(article);
    video.autoplay = true;
    video.muted = true;
    video.loop = true;
  ...
  async function handleOpenedModalOnce(modalEl) {
    ...
    // ID оставляем только для поиска слайдера (как раньше)
    const productId = modalEl.getAttribute("data-id");
    if (!productId) return;

    // Артикул — ключ видео
    const article = readArticleFromModal(modalEl);
    if (!article) return;
    ...
    // seen-метка: чтобы не крутить одно и то же каждые 300мс
    // но если артикул изменился (теоретически) — разрешаем заново
    const seenVal = sliderEl.getAttribute(CFG.modalSeenAttr);
    if (seenVal === article) return;
    sliderEl.setAttribute(CFG.modalSeenAttr, article);

    const chk = await checkVideoExistsByArticle(article);
    if (!chk.exists) return;

    const res = ensureModalVideoSlideByArticle(article, sliderEl);
    if (!res.ok) return;
  ...
  // Модалка: лёгкий polling (seen по артикулу + кеш HEAD → без спама)
